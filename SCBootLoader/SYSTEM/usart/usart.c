...
void MyDMA_Config()
{
	...
	DMAInitStructure.DMA_Memory0BaseAddr = (uint32_t)(&USART_RX_BUF[0]);
	DMAInitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
	DMAInitStructure.DMA_BufferSize = USART_REC_LEN;
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	else if(USART_GetITStatus(USART1,USART_IT_IDLE)!=RESET)
	{
		  ...
		  ValidDataLength=USART_REC_LEN-DMA_GetCurrDataCounter(DMA2_Stream2);
...
void UsartService()
{
	if((ValidDataLength>1)&&(RecFlag==1))
	{
		printf("%d---%s\r\n",ValidDataLength,USART_RX_BUF);
		ValidDataLength=0;
	}
}
 void Usart_SendData(u8 *Command,u8 Length)
{
	u8 i=0;
	while(i<Length)
	{
